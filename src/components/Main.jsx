...
const Main = () => {
    ...
    const handlePopup = async (detail) => {
    showPopup(!popup)
    setRackDetails({...rackDetails, name: detail})
    try {
        const response = await axios.get('api/rack')
        const search_data = response.data.search_data
        setRackDetails({...rackDetails, items: search_data})
    ...
    return (
      ...
      <Popup show={popup} close={showPopup} rackName={rackDetails.name} rackItems={rackDetails.items} />
      </div>
    </div>
    )
}
